...
### d108: No target question counts; coverage drives, count is an output
**Date:** 2026-06-09
Surfaced by Smith and the Topic 1 author during the Forces vocabulary-proposal turn. Briefs do not set target question counts (total or per-subtag) and authors do not invent counts for themselves. Coverage is driven by fully testing each in-scope spec point from the angles the past papers use and could use, atomised; the resulting total is an output, never a target and never a stopping condition. Two failure modes ruled out: "I have hit the number, therefore I am done" (stop short of fully testing an idea) and "I have not hit the number, therefore I am not done" (pad with weak items). Extends d049 (type-mix percentages are a guideline) and d016 (genuine-variety rule) from "do not pad to hit the mix" to "do not author toward any number, including a total". Supersedes the "negotiate a target volume" framing in d019 (per-subtopic counts are provisional, calibration batch first) and d021 (Topic 7 target about 80 to 100 base questions); calibration batches still happen but they are sanity-glances, not count-negotiation. Applies to Topic 1 Forces and every subsequent commission; the brief template (each new AUTHOR_BRIEF_*.md) carries this convention from now on.

### d109: Forces vocab approved (three groups, sixteen subtags); units stays standalone, mechanics_equations folds into subtags
**Date:** 2026-06-09
The Forces author's `topic1_vocabulary_proposal.md` is approved as proposed. Three parent groups (`units`, `movement_position`, `forces_movement`) mirror the 4SS0 Single Award spec sub-topics; sixteen child subtags each anchored on a spec point; three cross-cutting tags (`definition`, `practical_skill`, `calc`). The boundary table in §5 of the proposal is ratified as the in-scope cut and the routing for carved-out content. Two specific rulings: (1) `units` stays a standalone parent group (not folded into calc coverage as Topic 8 did for 8.1) because mechanics has seven distinct units across distinct quantities and both-directions recall is genuinely testable; per d084 (registry-worthiness is shape, not topic volume) this is shape-justified. (2) `mechanics_equations` is NOT built as a dedicated registry; per-equation atoms live inside their conceptual subtags (a=(v-u)/t atoms in `acceleration`, F=ma atoms in `newtons_second_law`, etc) to avoid duplicating the subtag structure and to avoid the matrix-completeness drift d077 named on nebula_colour. A dedicated equations registry can emerge later if cross-equation questions surface a need for one. The four registry candidates that do build: `motion_graphs`, `force_types`, `stopping_distance_factors`, `mechanics_units`.
