...
## 2026-06-12 - Smith review: borrow the Trilogy forces widgets instead of hand-drawn stage 2

Smith judged the hand-drawn stage-2 renderers (car, light-gate apparatus, stick figures, lift) too crude and pointed at the Trilogy 6.5 Forces work. Investigation: the Trilogy `Forces authoring` chat does not draw its own diagrams; it consumes a Widgets seat's purpose-built library at `C:\Claude (not on Gdrive, nor OneDrive)\Trilogy Physics\app\widgets\`. Two self-contained files, no external deps: `widgets_core.js` (542 lines) + `forces-diagrams.js` (2108 lines). Palette is hardcoded but identical to ours (#1a1a17 / #fffdf6 / #b03030 / #8c8579 / #4d4943 / #faf6ed), so it themes natively. It registers `window.TOPIC_DIAGRAMS[kind]`: motion_graph, area_under_vt, gradient_tool, braking_vt, stopping_distance_speed, free_body_diagram, ramp_fbd (inclined plane), vector_addition, vector_resolve, spring_extension, collision_illustration, each with correct and distractor variants (the d036 per-option-diagram capability). It does NOT cover acceleration_apparatus (light gates) or object_scene.

DECISION (Smith 2026-06-12): borrow from the Trilogy widgets rather than ship my hand-drawn stage 2. INTERIM (Smith): free_body_diagram / acceleration_apparatus / object_scene keep the clean dashed placeholder in the live tool until the borrowed versions land; nothing crude ships. Stage 1 graph family stays deployed (Smith was happy with it). Stage-2 hand-drawn code is parked in the scratchpad, not deployed.

Cross-project note: this borrows a Trilogy peer-project artefact; per the protocol it should be acknowledged via the GCSE Physics Overview seat. Open questions for architecture before the port: (a) wholesale adopt both files + bridge the engine dispatch to window.TOPIC_DIAGRAMS (matches the original BRIEF_diagrams_chat TOPIC_DIAGRAMS pattern, gets every widget) versus selectively lift just free_body_diagram / ramp_fbd / spring_extension / vector_* into forces_diagrams.js; (b) kind-name reconciliation, the Pre-IB AUTHOR_BRIEF_topic1_forces.md §9 names velocity_time_graph etc. while Trilogy uses motion_graph etc., so either re-point the author brief or add alias renderers; (c) whether acceleration_apparatus and object_scene (not in the Trilogy set) are still wanted, and if so rebuilt to a higher standard.
